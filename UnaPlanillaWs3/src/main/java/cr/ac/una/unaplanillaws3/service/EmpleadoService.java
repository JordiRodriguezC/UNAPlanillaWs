...
public class EmpleadoService {
    ...
    public Respuesta validarUsuario(String usuario, String clave) {
        try {
            ...
            qryActividad.setParameter("empUsuario", usuario);
            qryActividad.setParameter("empClave", clave);
    ...
    public Respuesta getEmpleadosFilter() {
        try {
            Query qryEmpleado = em.createNamedQuery("Empleado.findAll", Empleado.class);
            List<Empleado> empleados = qryEmpleado.getResultList();
            List<EmpleadoDto> empleadosDto = new ArrayList<>();
            empleados.stream().forEach(x -> {
                empleadosDto.add(new EmpleadoDto(x));
            });
            return new Respuesta(true, CodigoRespuesta.CORRECTO, "", "", "Empleados", empleadosDto);
    ...
    public Respuesta guardarEmpleado(EmpleadoDto empleadoDto) {
        try {
            Empleado empleado;
            if (empleadoDto.getEmpId() != null && empleadoDto.getEmpId() > 0) {
                empleado = em.find(Empleado.class, empleadoDto.getEmpId());
                if (empleado == null) {
